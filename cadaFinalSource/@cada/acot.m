function y = acot(x)
%CADA overloaded operation for acot
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = acot(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = acot(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -',x.derivative{Vcount,1},'./(cadatf1(:).^2+1);\n']);
