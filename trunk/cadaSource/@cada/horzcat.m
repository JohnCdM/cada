...
for Inputcount = 2:NUMinput;
    ...
    NcolMAP(1,Inputcount) = size(yfun,2);
end
    ...
    for Inputcount = 2:NUMinput;
        if isa(varargin{Inputcount},'cada')
            if ~isempty(varargin{Inputcount}.derivative{Acount,1}) && ~isempty(y.variable{Acount,1})
                varargin{Inputcount}.derivative{Acount,2}(:,2) = varargin{Inputcount}.derivative{Acount,2}(:,2) + NcolMAP(Inputcount-1);
