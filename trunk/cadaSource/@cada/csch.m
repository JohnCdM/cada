function y = csch(x)
%CADA overloaded operation for csch
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = csch(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = csch(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -coth(cadatf1(:)).*csch(cadatf1(:))',x.derivative{Vcount,1},';\n']);
