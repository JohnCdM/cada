...
for FIcount = 1:FMrow;
    for FJcount = 1:FNcol;
        ...
        y.function{FIcount,FJcount} = ['acosh(',x.function{FIcount,FJcount},')'];
    end
end
    ...
    if ~isempty(x.derivative{Acount,1})
        ...
        for Dcount = 1:nz;
            ...
            if strcmp(x.derivative{Acount,1}{Dcount,1},'1')
                y.derivative{Acount,1}{Dcount,1} = ['1./sqrt((',x.function{FI,FJ},').^2 - 1)'];
            elseif strcmp(x.derivative{Acount,1}{Dcount,1},'-1')
                y.derivative{Acount,1}{Dcount,1} = ['-1./sqrt((',x.function{FI,FJ},').^2 - 1)'];
            else
                y.derivative{Acount,1}{Dcount,1} = ['1./sqrt((',x.function{FI,FJ},').^2 - 1).*(',x.derivative{Acount,1}{Dcount,1},')'];
            end
        end
