            ...
            for Dcount = 1:unz;
                ...
                if ~isempty(uderivative{Dcount,1}) && ~isempty(uderivative{Dcount,2})
                    ...
                    if strcmp(uderivative{Dcount,2},'-1')
                        z.derivative{Acount,1}{Dcount,1} = ['(',LS,' + ',RS,')./((',y.function{FI,FJ},').^2)'];
                    else
                        z.derivative{Acount,1}{Dcount,1} = ['(',LS,' - (',RS,'))./((',y.function{FI,FJ},').^2)'];
                    end
                elseif ~isempty(uderivative{Dcount,1})
                    % rep x
                    if strcmp(uderivative{Dcount,1},'1')
                        z.derivative{Acount,1}{Dcount,1} = ['1./(',y.function{FI,FJ},')'];
                    elseif strcmp(uderivative{Dcount,1},'-1')
                        z.derivative{Acount,1}{Dcount,1} = ['-1./(',y.function{FI,FJ},')'];
                    else
                ...
                elseif ~isempty(uderivative{Dcount,2})
                    % rep y
                    if strcmp(uderivative{Dcount,2},'1')
                        z.derivative{Acount,1}{Dcount,1} = ['-(',x.function{FI,FJ},')./((',y.function{FI,FJ},').^2)'];
                    elseif strcmp(uderivative{Dcount,2},'-1')
                        z.derivative{Acount,1}{Dcount,1} = ['(',x.function{FI,FJ},')./((',y.function{FI,FJ},').^2)'];
                    else
                        z.derivative{Acount,1}{Dcount,1} = ['-(',x.function{FI,FJ},')./((',y.function{FI,FJ},').^2).*(',uderivative{Dcount,2},')'];
                    end
        ...
        elseif ~isempty(y.derivative{Acount,1})
            ...
            for Dcount = 1:nz;
                ...
                elseif strcmp(y.derivative{Acount,1}{Dcount,1},'-1')
                    ...
                    z.derivative{Acount,1}{Dcount,1} = ['-(',x.function{FI,FJ},')./((',y.function{FI,FJ},').^2).*(',y.derivative{Acount,1}{Dcount,1},')'];
                end
            end
    ...
    for Acount = 1:NUMvar;
        if ~isempty(x.derivative{Acount,1})
            ...
            for Dcount = 1:nz;
                ...
                FJ = x.derivative{Acount,2}(Dcount,2);
                if strcmp(x.derivative{Acount,1}{Dcount,1},'1')
                    z.derivative{Acount,1}{Dcount,1} = ['1./(',ystr{FI,FJ},')'];
                elseif strcmp(x.derivative{Acount,1}{Dcount,1},'-1')
                    z.derivative{Acount,1}{Dcount,1} = ['-1./(',ystr{FI,FJ},')'];
                else
    ...
    for Acount = 1:NUMvar;
        if ~isempty(y.derivative{Acount,1})
            ...
            for Dcount = 1:nz;
                ...
                FJ = y.derivative{Acount,2}(Dcount,2);
                if strcmp(y.derivative{Acount,1}{Dcount,1},'1')
                    z.derivative{Acount,1}{Dcount,1} = ['-(',xstr{FI,FJ},')./((',y.function{FI,FJ},').^2)'];
                elseif strcmp(y.derivative{Acount,1}{Dcount,1},'-1')
                    z.derivative{Acount,1}{Dcount,1} = ['(',xstr{FI,FJ},')./((',y.function{FI,FJ},').^2)'];
                else
                    z.derivative{Acount,1}{Dcount,1} = ['-(',xstr{FI,FJ},')./((',y.function{FI,FJ},').^2).*(',y.derivative{Acount,1}{Dcount,1},')'];
                end
            end
            z.derivative{Acount,2} = y.derivative{Acount,2};
        end
