...
for FIcount = 1:FMrow;
    for FJcount = 1:FNcol;
        ...
        y.function{FIcount,FJcount} = ['log(',x.function{FIcount,FJcount},')'];
    end
end
    ...
    if ~isempty(x.derivative{Acount,1})
        ...
        for Dcount = 1:nz;
            ...
            if strcmp(x.derivative{Acount,1}{Dcount,1},'1')
                y.derivative{Acount,1}{Dcount,1} = ['1./(',x.function{FI,FJ},')'];
            elseif strcmp(x.derivative{Acount,1}{Dcount,1},'-1')
                y.derivative{Acount,1}{Dcount,1} = ['-1./(',x.function{FI,FJ},')'];
            else
                y.derivative{Acount,1}{Dcount,1} = ['1./(',x.function{FI,FJ},').*(',x.derivative{Acount,1}{Dcount,1},')'];
            end
        end
