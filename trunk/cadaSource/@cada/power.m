            ...
            for Dcount = 1:unz;
                ...
                if ~isempty(uderivative{Dcount,1}) && ~isempty(uderivative{Dcount,2})
                    ...
                    if strcmp(uderivative{Dcount,1},'1')
                        LS = ['(',y.function{FI,FJ},')./(',y.function{FI,FJ},')'];
                    elseif strcmp(uderivative{Dcount,1},'-1')
                        LS = ['-(',y.function{FI,FJ},')./(',y.function{FI,FJ},')'];
                    else
                        LS = ['(',y.function{FI,FJ},')./(',y.function{FI,FJ},').*(',uderivative{Dcount,1},')'];
                    end
                    if strcmp(uderivative{Dcount,2},'1') || strcmp(uderivative{Dcount,2},'-1')
                        RS = ['log(',x.function{FI,FJ},')'];
                    else
                        RS = ['log(',x.function{FI,FJ},').*(',uderivative{Dcount,2},')'];
                    end
                    if strcmp(uderivative{Dcount,2},'-1')
                        z.derivative{Acount,1}{Dcount,1} = ['(',x.function{FI,FJ},').^(',y.function{FI,FJ},').*(',LS,' - ',RS,')'];
                    else
                        z.derivative{Acount,1}{Dcount,1} = ['(',x.function{FI,FJ},').^(',y.function{FI,FJ},').*(',LS,' + ',RS,')'];
                    end
                elseif ~isempty(uderivative{Dcount,1})
                    % rep x
                    if strcmp(uderivative{Dcount,1},'1')
                        z.derivative{Acount,1}{Dcount,1} = ['(',y.function{FI,FJ},').*(',x.function{FI,FJ},').^(',y.function{FI,FJ},'-1)'];
                    elseif strcmp(uderivative{Dcount,1},'-1')
                        z.derivative{Acount,1}{Dcount,1} = ['-(',y.function{FI,FJ},').*(',x.function{FI,FJ},').^(',y.function{FI,FJ},'-1)'];
                    else
                        z.derivative{Acount,1}{Dcount,1} = ['(',y.function{FI,FJ},').*(',x.function{FI,FJ},').^(',y.function{FI,FJ},'-1).*(',uderivative{Dcount,1},')'];
                    end
                elseif ~isempty(uderivative{Dcount,2})
                    % rep y
                    if strcmp(uderivative{Dcount,2},'1')
                        z.derivative{Acount,1}{Dcount,1} = ['(',x.function{FI,FJ},').^(',y.function{FI,FJ},').*log(',x.function{FI,FJ},')'];
                    elseif strcmp(uderivative{Dcount,2},'-1')
                        z.derivative{Acount,1}{Dcount,1} = ['-(',x.function{FI,FJ},').^(',y.function{FI,FJ},').*log(',x.function{FI,FJ},')'];
                    else
                        z.derivative{Acount,1}{Dcount,1} = ['(',x.function{FI,FJ},').^(',y.function{FI,FJ},').*log(',x.function{FI,FJ},').*(',uderivative{Dcount,2},')'];
                    end
                end
            end
        ...
        if ~isempty(x.derivative{Acount,1})
            ...
            for Dcount = 1:nz;
                ...
                if strcmp(x.derivative{Acount,1}{Dcount,1},'1')
                    z.derivative{Acount,1}{Dcount,1} = ['(',ystr{FI,FJ},').*(',x.function{FI,FJ},').^(',ystrm1{FI,FJ},')'];
                elseif strcmp(x.derivative{Acount,1}{Dcount,1},'-1')
                    z.derivative{Acount,1}{Dcount,1} = ['-(',ystr{FI,FJ},').*(',x.function{FI,FJ},').^(',ystrm1{FI,FJ},')'];
                else
                    z.derivative{Acount,1}{Dcount,1} = ['(',ystr{FI,FJ},').*(',x.function{FI,FJ},').^(',ystrm1{FI,FJ},').*(',x.derivative{Acount,1}{Dcount,1},')'];
                end
            end
            z.derivative{Acount,2} = x.derivative{Acount,2};
        end
    ...
    for Acount = 1:NUMvar;
        if ~isempty(y.derivative{Acount,1})
            ...
            for Dcount = 1:nz;
                ...
                if strcmp(y.derivative{Acount,1}{Dcount,1},'1')
                    z.derivative{Acount,1}{Dcount,1} = ['(',xstr{FI,FJ},').^(',y.function{FI,FJ},').*log(',xstr{FI,FJ},')'];
                elseif strcmp(y.derivative{Acount,1}{Dcount,1},'-1')
                    z.derivative{Acount,1}{Dcount,1} = ['-(',xstr{FI,FJ},').^(',y.function{FI,FJ},').*log(',xstr{FI,FJ},')'];
                else
                    z.derivative{Acount,1}{Dcount,1} = ['(',xstr{FI,FJ},').^(',y.function{FI,FJ},').*log(',xstr{FI,FJ},').*(',y.derivative{Acount,1}{Dcount,1},')'];
                end
