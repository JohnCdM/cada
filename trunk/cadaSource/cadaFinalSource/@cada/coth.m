function y = coth(x)
%CADA overloaded operation for coth
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = coth(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = coth(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -csch(cadatf1(:)).^2.*',x.derivative{Vcount,1},';\n']);
