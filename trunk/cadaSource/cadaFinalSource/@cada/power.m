...
%CADA overloaded operation for power
    ...
    elseif isnumeric(y.function{1})
        ...
        for Vcount = 1:NUMvar;
            if ~isempty(x.derivative{Vcount,1}) && ~isempty(y.derivative{Vcount,1})
                ...
                fprintf(fid,['cadatf3 = ',x.function{1},'(cadatfind2);\n']);
                fprintf(fid,['cadatf4 = ',y.function{1},'(cadatfind2);\n']);
                if length(yind) > length(xind)
                    fprintf(fid,[derivstr,'(cadadind2) = cadatf3(:).^cadatf4(:).*log(cadatf3(:)).*',y.derivative{Vcount,1},';\n']);
                    fprintf(fid,[derivstr,'(cadadind1) = ',derivstr,'(cadadind1) + cadatf1(:).^(cadatf2(:)-1).*cadatf2(:).*',x.derivative{Vcount,1},';\n']);
                else
                    fprintf(fid,[derivstr,'(cadadind1) = cadatf1(:).^(cadatf2(:)-1).*cadatf2(:).*',x.derivative{Vcount,1},';\n']);
                    fprintf(fid,[derivstr,'(cadadind2) = ',derivstr,'(cadadind2) + cadatf3(:).^cadatf4(:).*log(cadatf3(:)).*',y.derivative{Vcount,1},';\n']);
                end
                GLOBALCADA.LINECOUNT = GLOBALCADA.LINECOUNT+11;
            elseif ~isempty(x.derivative{Vcount,1})
                ...
                fprintf(fid,[';\ncadatf1 = ',x.function{1},'(cadatfind1);\n']);
                fprintf(fid,['cadatf2 = ',y.function{1},'(cadatfind1);\n']);
                fprintf(fid,[derivstr,' = cadatf1(:).^(cadatf2(:)-1).*cadatf2(:).*',x.derivative{Vcount,1},';\n']);
            ...
            elseif ~isempty(y.derivative{Vcount,1})
                ...
                fprintf(fid,[';\ncadatf1 = ',x.function{1},'(cadatfind1);\n']);
                fprintf(fid,['cadatf2 = ',y.function{1},'(cadatfind1);\n']);
                fprintf(fid,[derivstr,' = cadatf1(:).^cadatf2(:).*log(cadatf1(:)).*',y.derivative{Vcount,1},';\n']);
                GLOBALCADA.LINECOUNT = GLOBALCADA.LINECOUNT+4;
    ...
    if scalarflag == 1
        ...
        for Vcount = 1:NUMvar;
            if ~isempty(x.derivative{Vcount,1})
                ...
                cadaindprint(x.derivative{Vcount,2}(:,1),fid);
                fprintf(fid,');\n');
                fprintf(fid,[derivstr,' = ',num2str(y,16),'.*cadatf1(:).^',num2str(y-1,16),'.*',x.derivative{Vcount,1},';\n']);
        ...
        for Vcount = 1:NUMvar;
            if ~isempty(x.derivative{Vcount,1})
                ...
                if ~isempty(dxind)
                    ...
                    fprintf(fid,[derivstr,' = cadatf3(:).*cadatf2(:).^(cadatf3(:)-1).*',x.derivative{Vcount,1},';\n']);
                    GLOBALCADA.LINECOUNT = GLOBALCADA.LINECOUNT+4;
    ...
    if scalarflag == 1
        ...
        for Vcount = 1:NUMvar;
            if ~isempty(y.derivative{Vcount,1})
                derivstr = ['dcadaf',OPstr,'fd',z.variable{Vcount,1}];
                z.derivative{Vcount,1} = derivstr;
                fprintf(fid,['cadatf1 = ',z.function{1},'(']);
                cadaindprint(y.derivative{Vcount,2}(:,1),fid);
                fprintf(fid,');\n');
                fprintf(fid,[derivstr,' = cadatf1(:).*log(',num2str(x,16),').*',y.derivative{Vcount,1},';\n']);
                GLOBALCADA.LINECOUNT = GLOBALCADA.LINECOUNT+2;
                z.derivative{Vcount,2} = y.derivative{Vcount,2};
            end
        end
        ...
        for Vcount = 1:NUMvar;
            if ~isempty(y.derivative{Vcount,1})
                ...
                cadaindprint(y.derivative{Vcount,2}(:,1),fid);
                fprintf(fid,';\n');
                fprintf(fid,['cadatf2 = ',z.function{1},'(cadatfind1);\n']);
                fprintf(fid,'cadatf3 = cadatf1(cadatfind1);\n');
                fprintf(fid,[derivstr,' = cadatf2(:).*log(cadatf3(:)).*',y.derivative{Vcount,1},';\n']);
                GLOBALCADA.LINECOUNT = GLOBALCADA.LINECOUNT+4;
                z.derivative{Vcount,2} = y.derivative{Vcount,2};
            end
        end
