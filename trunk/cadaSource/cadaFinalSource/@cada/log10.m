function y = log10(x)
%CADA overloaded operation for log10
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = log10(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = log10(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = (1/log(10))./cadatf1(:).*',x.derivative{Vcount,1},';\n']);
