function y = tan(x)
%CADA overloaded operation for tan
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = tan(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = sec(cadatf1(:)).^2.*',x.derivative{Vcount,1},';\n']);
