function y = cos(x)
%CADA overloaded operation for cos
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = cos(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -sin(cadatf1(:)).*',x.derivative{Vcount,1},';\n']);
