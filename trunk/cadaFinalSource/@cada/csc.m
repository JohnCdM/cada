function y = csc(x)
%CADA overloaded operation for csc
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = csc(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -cot(cadatf1(:)).*csc(cadatf1(:))',x.derivative{Vcount,1},';\n']);
