function y = exp(x)
%CADA overloaded operation for exp
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = exp(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = exp(cadatf1(:)).*',x.derivative{Vcount,1},';\n']);
