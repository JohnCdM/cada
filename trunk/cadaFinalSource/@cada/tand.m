function y = tand(x)
%CADA overloaded operation for tand
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = tand(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = tand(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = (180/pi)*secd(cadatf1(:)).^2.*',x.derivative{Vcount,1},';\n']);
