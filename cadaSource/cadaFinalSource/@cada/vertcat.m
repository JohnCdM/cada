...
%CADA overloaded operation for vertcat
...
xMrows = zeros(1,NUMinput);
Fstr = cell(1,NUMinput);
nzd = cell(1,NUMinput);
if isa(varargin{1},'cada')
    xNcol = varargin{1}.function{2}(2);
    xMrows(1) = varargin{1}.function{2}(1);
    ...
    xNcol = size(varargin{1},2);
    xMrows(1) = size(varargin{1},1);
    Fstr{1} = varargin{1};
    y.variable = [ ];
    y.function = cell(1,3);
end
...
for incount = 2:NUMinput
    if isa(varargin{incount},'cada')
        if varargin{incount}.function{2}(2) ~= xNcol
            error('CAT arguments dimensions are not consistent.');
        end
        xMrows(incount) = varargin{incount}.function{2}(1);
        ...
        if size(varargin{incount},2) ~= xNcol
            error('CAT arguments dimensions are not consistent.');
        end
        xMrows(incount) = size(varargin{incount},1);
        Fstr{incount} = varargin{incount};
    end
end
...
for incount = 1:NUMinput
    if isempty(nzd{1,incount})
        ...
        Fstr{1,incount} = ['cadatf',num2str(TVcount)];
        TVcount = TVcount+1;
    end
end
% print out function
fprintf(fid,['cadaf',OPstr,'f = vertcat(']);
for incount = 1:NUMinput
    if incount ~= NUMinput
        ...
        for incount = 1:NUMinput
            if ~isempty(nzd{incount})
                if nzd{incount}(Vcount) ~= 0
                    nzy = nnz(yrows);
                    if incount == 1
                        rind{1} = ytemp(1:xMrows(1),:);
                    else
                        rind{incount} = ytemp(sum(xMrows(1:incount-1))+1:sum(xMrows(1:incount)),:);
                    end
                    yrows(nzy+1:nzy+nzd{incount}(Vcount)) = rind{incount}(varargin{incount}.derivative{Vcount,2}(:,1));
                    ycols(nzy+1:nzy+nzd{incount}(Vcount)) = varargin{incount}.derivative{Vcount,2}(:,2);
                end
            end
        end
        nv = y.variable{Vcount,2}(1)*y.variable{Vcount,2}(2);
        dy = sparse(yrows,ycols,ones(nzt(Vcount),1),FMrow*FNcol,nv);
        [yrows,ycols] = find(dy);
        if size(yrows,2) > 1
            yrows = yrows';
            ycols = ycols';
