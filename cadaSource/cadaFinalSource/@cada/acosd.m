function y = acosd(x)
%CADA overloaded operation for acosd
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = acosd(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = acosd(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -(180/pi)*',x.derivative{Vcount,1},'./sqrt(1-cadatf1(:).^2);\n']);
