function y = acos(x)
%CADA overloaded operation for acos
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = acos(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = acos(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = -',x.derivative{Vcount,1},'./sqrt(1-cadatf1(:).^2);\n']);
