function y = acosh(x)
%CADA overloaded operation for acosh
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = acosh(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = acosh(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = ',x.derivative{Vcount,1},'./sqrt(cadatf1(:).^2-1);\n']);
