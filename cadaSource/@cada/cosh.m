function y = cosh(x)
%CADA overloaded operation for cosh
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = cosh(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = cosh(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = sinh(cadatf1(:)).*',x.derivative{Vcount,1},';\n']);
