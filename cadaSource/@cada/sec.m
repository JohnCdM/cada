function y = sec(x)
%CADA overloaded operation for sec
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = sec(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = tan(cadatf1(:)).*sec(cadatf1(:)).*',x.derivative{Vcount,1},';\n']);
