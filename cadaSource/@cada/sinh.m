function y = sinh(x)
%CADA overloaded operation for sinh
...
if isnumeric(x.function{1})
    ...
    y.function = cell(1,2);
    y.function{1} = sinh(x.function{1});
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = sinh(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = cosh(cadatf1(:)).*',x.derivative{Vcount,1},';\n']);
