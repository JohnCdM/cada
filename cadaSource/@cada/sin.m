function y = sin(x)
%CADA overloaded operation for sin
...
% build y function
y.function = cell(1,3);
fprintf(fid,['cadaf',OPstr,'f = sin(',x.function{1},');\n']);
...
for Vcount = 1:NUMvar
    if ~isempty(x.derivative{Vcount,1})
        ...
        fprintf(fid,');\n');
        fprintf(fid,[derivstr,' = cos(cadatf1(:)).*',x.derivative{Vcount,1},';\n']);
